                    ...
                    <form action="{{ route('admin.mahasiswa.store') }}" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <textarea name="judul_skripsi" id="judul_skripsi" rows="3"
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all resize-none">{{ old('judul_skripsi') }}</textarea>
                        </div>
                            ...
                            <div>
                                ...
                                <select name="pembimbing_1_id" id="pembimbing_1_id" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all">
                                    ...
                                    @foreach($dosens as $dosen)
                                        <option value="{{ $dosen->id }}" {{ old('pembimbing_1_id') == $dosen->id ? 'selected' : '' }}>{{ $dosen->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="pembimbing_2_id" id="pembimbing_2_id" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none transition-all">
                                    ...
                                    @foreach($dosens as $dosen)
                                        <option value="{{ $dosen->id }}" {{ old('pembimbing_2_id') == $dosen->id ? 'selected' : '' }}>{{ $dosen->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
