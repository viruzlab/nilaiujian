<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Jadwal Sidang</h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('admin.jadwal.update', $jadwal->id) }}" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <select name="mahasiswa_id" id="mahasiswa_id" required
                                ...
                                @foreach($mahasiswas as $mhs)
                                    <option value="{{ $mhs->id }}" {{ old('mahasiswa_id', $jadwal->mahasiswa_id) == $mhs->id ? 'selected' : '' }}>{{ $mhs->nim }} - {{ $mhs->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="datetime-local" name="waktu_sidang" id="waktu_sidang" value="{{ old('waktu_sidang', date('Y-m-d\TH:i', strtotime($jadwal->waktu_sidang))) }}" required
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all">
                        </div>
                        <div>
                            <label for="ruangan" class="block text-sm font-medium text-gray-700 mb-1">Ruangan</label>
                            <input type="text" name="ruangan" id="ruangan" value="{{ old('ruangan', $jadwal->ruangan) }}"
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
                                placeholder="Contoh: Ruang 301">
                        </div>
                        <div>
                            <label for="kelompok_ujian" class="block text-sm font-medium text-gray-700 mb-1">Kelompok Ujian</label>
                            <input type="text" name="kelompok_ujian" id="kelompok_ujian" value="{{ old('kelompok_ujian', $jadwal->kelompok_ujian) }}"
                                class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all"
                                placeholder="Contoh: Gelombang 1" list="kelompok_list">
                        ...
                        <div class="flex items-center space-x-3 pt-2">
                            <button type="submit" class="bg-emerald-600 text-white px-6 py-2.5 rounded-lg hover:bg-emerald-700 transition-colors font-medium">Simpan Perubahan</button>
