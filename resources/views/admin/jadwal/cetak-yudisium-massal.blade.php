...
<head>
    ...
    <title>Pengumuman Yudisium Massal {{ $selectedKelompok ? '- Kelompok ' . $selectedKelompok : '' }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="print-bar no-print">
        <div class="info">
            <strong>📄 Pengumuman Yudisium Massal</strong> {{ $selectedKelompok ? ' - Kelompok ' . $selectedKelompok : '(Semua)' }}
            ({{ count($jadwalsSiapCetak) }} Data)
        </div>
        <div class="btn-group">
    ...
    <div class="page">
        ...
        <div class="title">
            ...
            <div class="nomor">Nomor: {{ $data['nomorSurat'] }}/UN40.A7.5.7/PK.03.06/{{ date('Y') }}</div>
        </div>
            ...
            <div class="data-mhs">
                <table>
                    <tr>
                        <td>Nama</td>
                        <td>:</td>
                        <td><strong>{{ optional($data['jadwal']->mahasiswa)->nama ?? '-' }} /
                                {{ optional($data['jadwal']->mahasiswa)->nim ?? '-' }}</strong></td>
                    </tr>
                    <tr>
                        <td>Program Studi</td>
                        <td>:</td>
        ...
        <div class="status-box">
            <div class="status-row">
                <div class="status-cell" style="{{ !$data['isLulus'] ? 'text-decoration: line-through;' : '' }}">
                    LULUS
                </div>
                <div class="status-cell" style="{{ $data['isLulus'] ? 'text-decoration: line-through;' : '' }}">
                    TIDAK LULUS
        ...
        <div class="lulusan-box">
            <div class="lulusan-title">Lulusan Program Studi Ilmu Ekonomi dan Keuangan Islam Ke - <strong>{{ $data['lulusanKe'] }}</strong></div>
            <div class="ipk-row">
                <div class="ipk-item">
                    <span class="ipk-label">IPK</span>
                    <span class="ipk-colon">:</span>
                    <span class="ipk-value">{{ number_format($data['nilaiAkhirAngka'], 2) }}</span>
                </div>
                <div class="ipk-item">
                    <span class="ipk-label">Predikat</span>
                    <span class="ipk-colon">:</span>
                    <span class="ipk-value">{{ $data['mutuAkhirPredikat'] }}</span>
                </div>
            </div>
        </div>
